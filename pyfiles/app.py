...
def UpdateFinalCraft():
    if potionTypeCrafting == "wildSage_Bool":
        ...
        craftingFinal.set(
            "Removes: " + diceRolled.get() + "d" + diceAmount.get() + " rounds of poison in the target’s system")
        craftingDC.set(finalDC.get())
    else:
        ...
        if carryCheck(int(mat4Geng.get()), int(usedGengko.get()), labelGengko1):
            if craftingLevelCheck() == False:
                if int(usedGengko.get()) == 0:
                    ...
                    UpdateFinalCraft()
                    crafting_mod_cur += 1
                    AddtionalInformationAdder("\nDivide total by 2, receive that amount per round for 2 rounds")
        ...
        if carryCheck(int(mat6Emetic.get()), int(usedEmetic.get()), labelEmetic1):
            if craftingLevelCheck() == False:
                if int(usedEmetic.get()) == 0:
                    ...
                    UpdateFinalCraft()
                    crafting_mod_cur += 1
                    AddtionalInformationAdder("\nDelay the Effect this was combined with by 1d6 rounds")
...
def highlightcleanup():
    ReturnColorChange(label40, label41, label42, label108, label109, label110, label105, label106, label107,
                      ...
                      labelGengko1, labelGengko2, labelGengko3, usedGengkoLabel, labelChromus1, labelChromus2,
                      labelChromus3, usedChromusLabel, labelManPot,
                      labelManPot2, labelManPot3, labelHyanPot, labelHyanPot2, labelHyanPot3, labelFennPot,
                      labelFennPot2, labelFennPot3, labelBloodPot, labelBloodPot2,
                      labelBloodPot3, labelMilkweed1, labelMilkweed2, labelMilkweed3, usedMilkweedLabel, labelEmetic1,
                      labelEmetic2, labelEmetic3, usedEmeticLabel)
...
button_craft_lvl1 = tk.Button(frame, text="lvl1",
                              command=lambda: CraftingLevelButton(button_craft_lvl1, label_craft_mod, "Max 3", 3,
                                                                  button_craft_lvl2, button_craft_lvl3))
button_craft_lvl1.grid(row=16, column=1)
button_craft_lvl2 = tk.Button(frame, text="lvl2",
                              command=lambda: CraftingLevelButton(button_craft_lvl2, label_craft_mod, "Max 6", 6,
                                                                  button_craft_lvl1, button_craft_lvl3))
button_craft_lvl2.grid(row=16, column=2)
button_craft_lvl3 = tk.Button(frame, text="lvl3",
                              command=lambda: CraftingLevelButton(button_craft_lvl3, label_craft_mod, "Max 9", 9,
                                                                  button_craft_lvl1, button_craft_lvl2))
...
# List of buttons
button1 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry0.get(), 'Bloodgrass', Bloodgrassx), clearText(entry0)))
button1.grid(row=1, column=3)
button102 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry0.get(), 'Bloodgrass', Bloodgrassx), clearText(entry0)))
button102.grid(row=1, column=4)

button2 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry1.get(), 'ChromusSlime', ChromusSlimex), clearText(entry1)))
button2.grid(row=2, column=3)
button202 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry1.get(), 'ChromusSlime', ChromusSlimex), clearText(entry1)))
button202.grid(row=2, column=4)

button3 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry2.get(), 'DriedEphedra', DriedEphedrax), clearText(entry2)))
button3.grid(row=3, column=3)
button302 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry2.get(), 'DriedEphedra', DriedEphedrax), clearText(entry2)))
button302.grid(row=3, column=4)

button4 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry3.get(), 'EmeticWax', EmeticWaxx), clearText(entry3)))
button4.grid(row=4, column=3)
button402 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry3.get(), 'EmeticWax', EmeticWaxx), clearText(entry3)))
button402.grid(row=4, column=4)

button5 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry4.get(), 'FennelSilk', FennelSilkx), clearText(entry4)))
button5.grid(row=5, column=3)
button505 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry4.get(), 'FennelSilk', FennelSilkx), clearText(entry4)))
button505.grid(row=5, column=4)

button6 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry5.get(), 'GengkoBrush', GengkoBrushx), clearText(entry5)))
button6.grid(row=6, column=3)
button602 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry5.get(), 'GengkoBrush', GengkoBrushx), clearText(entry5)))
button602.grid(row=6, column=4)

button7 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry6.get(), 'HyancinthNectar', HyancinthNectarx), clearText(entry6)))
button7.grid(row=7, column=3)
button702 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry6.get(), 'HyancinthNectar', HyancinthNectarx), clearText(entry6)))
button702.grid(row=7, column=4)

button8 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry7.get(), 'LavenderSprig', LavenderSprigx), clearText(entry7)))
button8.grid(row=8, column=3)
button802 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry7.get(), 'LavenderSprig', LavenderSprigx), clearText(entry7)))
button802.grid(row=8, column=4)

button9 = tk.Button(frame, text="Add", bg='grey',
                    command=lambda: (addAmount(entry8.get(), 'MandrakeRoot', MandrakeRootx), clearText(entry8)))
button9.grid(row=9, column=3)
button902 = tk.Button(frame, text="Del", bg='grey',
                      command=lambda: (delAmount(entry8.get(), 'MandrakeRoot', MandrakeRootx), clearText(entry8)))
button902.grid(row=9, column=4)

button10 = tk.Button(frame, text="Add", bg='grey',
                     command=lambda: (addAmount(entry9.get(), 'MilkweedSeeds', MilkweedSeedsx), clearText(entry9)))
button10.grid(row=10, column=3)
button1002 = tk.Button(frame, text="Del", bg='grey',
                       command=lambda: (delAmount(entry9.get(), 'MilkweedSeeds', MilkweedSeedsx), clearText(entry9)))
button1002.grid(row=10, column=4)

button11 = tk.Button(frame, text="Add", bg='grey',
                     command=lambda: (addAmount(entry10.get(), 'WildSageroot', WildSagerootx), clearText(entry10)))
button11.grid(row=11, column=3)
button1102 = tk.Button(frame, text="Del", bg='grey',
